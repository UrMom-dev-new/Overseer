...
Registry, so you can run OVERSEER without building anything:

```bash
docker pull ghcr.io/aiacos/overseer:latest      # or a pinned tag, e.g. :0.1.0
docker run -d --name overseer \
  -p 3005:3000 --env-file .env --restart unless-stopped \
  ghcr.io/aiacos/overseer:latest
```
...
> (`docker build -t overseer:latest /path/to/overseer`) or set `image:` to a
