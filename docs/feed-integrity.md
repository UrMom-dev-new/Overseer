...
- Police surveillance capability records from `Ringmast4r/surveillance-capabilities-map` are reference/report records. EFF Atlas, USASpending, Washington Post 1033, and BuzzFeed flight-path data must not be displayed as live observations. City coordinates are source-derived where available; state centroid fallbacks are labeled `region` precision.
- Surveillance industry dossiers from `Ringmast4r/Surveillance-Industry` are Markdown reference records, not live observations. Country and regional markers are representative centroids with explicit precision labels; unavailable dossier files are omitted and reported through provider status.
- MAC vendor lookups use `Ringmast4r/OUI-Master-Database` reference records from its generated master CSV. Missing device type, country, registration date, or address fields remain `null`; unmatched prefixes return `Not Found` without guessing a manufacturer. `macvendors.co` is used only as a real alternate provider when the master CSV is unavailable or lacks a matching prefix.
- ODINT target records from `Ringmast4r/ODINT` are passive reference records parsed from public `CYBER RECON TOUR` text files. Only explicit domains and URLs are emitted. Comments, prose, relative paths, unavailable files, and failed GitHub raw fetches are omitted and reported through provider status instead of converted into synthetic targets.
- FED rolodex records from `Ringmast4r/FED` are Markdown reference records parsed from `spy-vs-spy.md` and `cultural-centers.md`. Overseer preserves source text as unassessed reference descriptions, keeps missing websites as `null`, and omits unavailable Markdown files instead of creating placeholder agencies or cultural centers.
- Global Data Center Map records from `Ringmast4r/Global-Data-Center-Map` are reference infrastructure records, not live operational telemetry. Overseer plots only valid source `Point` features from `datacenters.geojson`; catalog records that exist only in `datacenters.json` are counted in summaries but are not geocoded, jittered, or inferred. Coordinate precision is marked `unknown` because upstream coordinates may be building-level, city, state, or country centroids.
