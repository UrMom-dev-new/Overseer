...
function OverseerMap({ data, activeLayers, onEntityClick, onMouseCoords, onRightClick, onViewStateChange, initialView, flyToLocation, projection = 'globe', mapStyle = 'dark', sweepData, scanTargets = [], theme = 'core' }: OverseerMapProps) {
  ...
  useEffect(() => {
    ...
    map.on('load', () => {
      ...
      // Create icons — OVERSEER Unified Palette
      ...
      // ══ OVERSEER SDK — Lattice Intelligence Mesh ══
    ...
    ['sdk-sea','sdk-sea-glow','sdk-air','sdk-air-glow','sdk-intel','sdk-intel-glow'].forEach(layer => {
      map.on('click', layer, e => {
        ...
        const srcUrl = p.url || SDK_SOURCE_URLS[p.source] || 'https://overseerai.live';
        ...
        popup([coords.lng, coords.lat], `<div style="${pStyle}border:1px solid ${domainColor}40;">
          ...
          <div style="display:grid;grid-template-columns:1fr 1fr;gap:6px;font-size:9px;margin-bottom:8px;">
            ...
            <div><span style="color:#5C5A54;">SOURCE</span><br/><a href="${srcUrl}" target="_blank" style="color:${domainColor};text-decoration:underline;cursor:pointer;">${p.source || 'OVERSEER'}</a></div>
          </div>
  ...
  // ══ OVERSEER SDK — Lattice Sensor Mesh ══
  ...
  useEffect(() => {
    ...
    try {
      if (enabled) {
        ...
        if (!map.getSource('overseer-buildings')) {
          map.addSource('overseer-buildings', {
            type: 'vector',
        ...
        if (!map.getLayer('overseer-3d-buildings')) {
          map.addLayer({
            id: 'overseer-3d-buildings',
            source: 'overseer-buildings',
        ...
        // ── DISABLE 3D ──
        if (map.getLayer('overseer-3d-buildings')) map.removeLayer('overseer-3d-buildings');
      }
    } catch (e) {
      console.warn('[OVERSEER] 3D terrain toggle error:', e);
    }
