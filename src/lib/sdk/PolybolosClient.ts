 ...
 * ║                                                                 ║
 * ║  Subscribes to OVERSEER feeds + external providers (Lattice),     ║
...
export class PolybolosClient {
  ...
  /** Connect to the OVERSEER SSE stream endpoint */
  private connectSSE(): void {
    if (typeof EventSource === 'undefined') return;

    try {
      this.sseConnection = new EventSource(
        `${this.config.overseerBaseUrl}/api/sdk/stream`
      );
  ...
  /**
   * Ingest raw OVERSEER data and translate it into Polybolos entities.
   * This is the primary method called by page.tsx to feed data into the SDK.
   */
  ingestOverseerData(data: Record<string, any>): void {
  ...
  private getActiveFeedCount(): number {
    let count = 0;
    // Count OVERSEER feeds that have data
